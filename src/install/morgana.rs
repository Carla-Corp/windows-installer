...
pub fn install(sink: ExtEventSink, installation: &str) {
    ...
    let url = "https://github.com/lucasFelixSilveira/morgana/archive/refs/heads/main.zip";

    let mut response = reqwest::blocking::get(url).unwrap();
    let mut file = File::create("morgana.zip").unwrap();
    ...
    let zip_file = File::open("morgana.zip").unwrap();
    ...
    let status = Command::new("g++")
        .current_dir("temp/morgana-main/compiler")
        .args([
            "-std=c++17",
            "-g",
            "-O0",
